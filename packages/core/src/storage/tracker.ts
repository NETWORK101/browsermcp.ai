...
export class UsageTracker {
  ...
  todayUsage(): { sessions: number; tokens: number; estimatedCost: string } {
    const row = this.db
      ...
      .get() as { sessions: number; tokens: number };
  ...
  usage(
    days: number = 7,
  ): { sessions: number; tokens: number; estimatedCost: string } {
    const row = this.db
      .prepare(
  ...
  getSnapshot(
    url: string,
  ): { markdown: string; tokenCount: number; updatedAt: string } | null {
    const row = this.db
      .prepare(
        "SELECT markdown, token_count as tokenCount, updated_at as updatedAt FROM snapshots WHERE url = ?",
      )
      .get(url) as
      | { markdown: string; tokenCount: number; updatedAt: string }
      | undefined;
    return row ?? null;
  }
