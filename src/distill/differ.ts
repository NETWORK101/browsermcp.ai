 ...
 */
export function diffMarkdown(previous: string, current: string): DiffResult {
  ...
  stack.forEach((line, idx) => {
    if (line.type !== 'context') {
      for (let k = Math.max(0, idx - contextRadius); k <= Math.min(stack.length - 1, idx + contextRadius); k++) {
        changeIndices.add(k);
      }
    }
  });
